...
def send_wx_notification(title, message):
    ...
    """
    try:
        mydata = {"text": title, "desp": message}
...
def setup_logger():
    ...
    handler = RotatingFileHandler(
        "grid_trader.log", maxBytes=10 * 1024 * 1024, backupCount=5, encoding="utf-8"
    )
...
try:
    ...
    if (
        ...
        logger.info(
            f"设置持仓模式成功: {change_position_mode_response}|是否为单向持仓: {change_position_mode_response['dualSidePosition'] != True}"
        )
    else:
        send_wx_notification(
            f"设置持仓模式失败",
            f"设置持仓模式失败，错误: {change_position_mode_response}",
        )
...
def handle_message():
    try:
        ...
        logger.info(
            f"收到 {symbol} 的新交易参数请求: {json.dumps(data, ensure_ascii=False)}"
        )
        ...
        if position_qty != 0:
            ...
            if trading_pairs[symbol]["exit_order_id"] is not None:
                order_response = client.query_order(
                    symbol, trading_pairs[symbol]["exit_order_id"]
                )
                if (
                    order_response["status"] == "FILLED"
                    or order_response["status"] == "CANCELED"
                ):
                    # 已经成交的话应该就没仓位了
                    logger.info(
                        f"{symbol} | 上一次的出场单已经成交or已撤单, orderId: {trading_pairs[symbol]['exit_order_id']}"
                    )
                else:
                    ...
                    if cancel_response["status"] == "CANCELED":
                        ...
                        logger.error(
                            f"{symbol} | 上一次的出场单撤单失败，响应: {cancel_response}"
                        )
                        send_wx_notification(
                            f"{symbol} | 上一次的出场单撤单失败",
                            f"上一次的出场单撤单失败，响应: {cancel_response}",
                        )
            ...
            order_response = client.new_order(
                ...
                timeInForce="GTC",
                price=round(
                    open_price * (1 - exit_price_percent),
            ...
            if order_response["orderId"] is not None:
                ...
                logger.info(
                    f"{symbol} | 出场单已创建，ID: {trading_pairs[symbol]['exit_order_id']}"
                )
                send_wx_notification(
                    f"{symbol} | 出场单已创建",
                    f"出场单已创建，ID: {trading_pairs[symbol]['exit_order_id']}",
                )
            else:
                logger.info(f"{symbol} | 出场单创建失败，响应: {order_response}")
                send_wx_notification(
                    f"{symbol} | 出场单创建失败",
                    f"出场单创建失败，响应: {order_response}",
                )

        # 判断之前的限价单是否已经成交，如果没成交，先撤单
        if trading_pairs[symbol]["entry_order_id"] is not None:
            order_response = client.query_order(
                symbol, trading_pairs[symbol]["entry_order_id"]
            )
            if (
                order_response["status"] == "FILLED"
                or order_response["status"] == "CANCELED"
                ...
                if cancel_response["status"] == "CANCELED":
                    ...
                    send_wx_notification(
                        f"{symbol} | 入场单撤单失败",
                        f"入场单撤单失败，响应: {cancel_response}",
                    )

        # 无论之前的单子是否成交，都需要挂上一个新的入场单
        entry_price = round(
            open_price * (1 - entry_price_percent),
            symbol_tick_size[symbol]["tick_size"],
        )
        ...
        if order_response["orderId"] is not None:
            ...
            logger.info(
                f"{symbol} | 入场单已创建，ID: {trading_pairs[symbol]['entry_order_id']}"
            )
            send_wx_notification(
                f"{symbol} | 入场单已创建",
                f"入场单已创建，ID: {trading_pairs[symbol]['entry_order_id']}",
            )
        else:
            logger.error(f"{symbol} | 入场单创建失败，响应: {order_response}")
            send_wx_notification(
                f"{symbol} | 入场单创建失败", f"入场单创建失败，响应: {order_response}"
            )
...
def before_req():
    ...
    if request.method == "POST":
        ...
        if (
            ...
            and request.remote_addr not in ip_white_list
        ):
...
def update_config():
    try:
        ...
        # 统一设置成单向持仓
        try:
            change_position_mode_response = client.change_position_mode(
            ...
            if (
                ...
                send_wx_notification(
                    f"设置单向持仓失败",
                    f"设置单向持仓失败，错误: {change_position_mode_response}",
                )
        ...
        for position in position_response["positions"]:
            ...
            if position["symbol"] == symbol and position["positionAmt"] != "0":
                ...
                if order_response is not None:
                    if order_response["status"] == "NEW" or order_response["status"] == "FILLED":
                        logger.info(
                            f"{position['symbol']}市价止盈平仓成功:\n{order_response}"
                        )
                        return jsonify({"status": "success", "message": "市价止盈平仓成功"})
                    else:
                        logger.error(
                            f"{position['symbol']}市价止盈平仓失败:\n{order_response}"
                        )
                        return jsonify(
                            {"status": "error", "message": "市价止盈平仓失败"}
                        )
